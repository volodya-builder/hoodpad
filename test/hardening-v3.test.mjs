...
before(async () => {
  ...
  factory = await deploy(deployer, "LaunchpadFactoryV3", [deployer.address, migrator.address]);
...
test("кап создателя нельзя обойти прямой покупкой у пула", async () => {
  ...
  const pool = { address: poolAddr, abi: ART("BondingCurvePoolV3").abi };
...
test("запуск монет закрыт, пока владелец не вызвал initConfig (чужой createToken не запирает настройку)", async () => {
  ...
  const f = await deploy(deployer, "LaunchpadFactoryV3", [deployer.address, mig.address]);
