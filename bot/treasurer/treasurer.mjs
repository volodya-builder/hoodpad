      ...
      if (!pod.length) console.log("Арена: вчера не было подиума — выплат нет.");
        ...
        if (payouts.length) {
          ...
          const rep = { day: yesterday, ts: Date.now(), potEth: pot, payouts,
            reason: `Подиум арены за ${new Date(yesterday).toISOString().slice(0, 10)}: казна потратила 50% баланса (${pot.toFixed(6)} ETH), 70/20/10 между местами. Выкупленное сожжено.` };
