...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      [
        ...
        { k: T("Порог (монеты за валюту)", "Threshold (quote coins)", "阈值（计价货币代币）"), v: T("виртуальный резерв × 4 — столько акции, сколько стоили 4 ETH на момент добавления в список", "virtual reserve × 4 — the amount of the asset that 4 ETH was worth when it was listed", "虚拟储备 × 4——上架时价值 4 ETH 的该资产数量") },
  ...
  },
  {
    ...
    body: [
      ...
      T("Дивиденды: создатель может включить налог до 3% с каждой сделки. Он берётся в валюте кривой и распределяется по балансам держателей. Бот площадки выплачивает накопленное автоматически (см. «Автоматизация»); забрать вручную можно в любой момент.", "Dividends: the creator can enable a tax of up to 3% on every trade. It is collected in the curve asset and distributed pro-rata to holders. The platform bot pays out accrued dividends automatically (see “Automation”); manual claim is available any time.", "分红：创建者可开启每笔交易最高 3% 的税。税以曲线资产收取，并按持仓比例分配给持有者。平台机器人自动发放累积的分红（见“自动化”）；也可随时手动领取。"),
    ],
  },
  {
    ...
    body: [
      ...
      T("Доля создателя копится в пуле и забирается кнопкой на странице монеты. Остальные 30% тоже копятся в пуле; каждые 5 минут бот площадки забирает их в сплиттер (FeeSplitterV6), когда накопилось от $1, — и сплиттер в той же транзакции раскладывает всё по трём адресам: казна арены, казна выкупа hood, команда. У монет за валюту всё это происходит в валюте кривой. Забрать долю площадки из пула в сплиттер может кто угодно — деньги идут только по адресам, зашитым в контракты.", "The creator share accrues in the pool and is claimed with a button on the coin page. The remaining 30% accrues in the pool too; every 5 minutes the platform bot collects it into the splitter (FeeSplitterV6) once at least $1 has built up — and the splitter forwards it to the three addresses in the same transaction: arena treasury, hood buyback treasury, team. For quote coins all of this happens in the curve asset. Anyone can move the platform share from a pool to the splitter — the funds only go to the addresses hard-wired in the contracts.", "创建者份额在池中累积，在代币页面一键领取。其余 30% 同样先在池中累积；累积到 $1 后，平台机器人每 5 分钟将其收集到分配合约（FeeSplitterV6），后者在同一笔交易中转给三个地址：竞技场金库、hood 回购金库、团队。计价货币代币的这一切都以曲线资产进行。任何人都可以把平台份额从池子转到分配合约——资金只会流向合约中硬编码的地址。"),
    ],
  },
  {
