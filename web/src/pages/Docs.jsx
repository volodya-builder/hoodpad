...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      T("Доля создателя копится в пуле и забирается кнопкой на странице монеты. У ETH-монет остальные 30% пул сам отправляет в сплиттер (FeeSplitterV6) при каждой сделке; у монет за валюту они копятся в пуле, и бот забирает их в сплиттер, как только накопится от $0.10 — сплиттер в той же транзакции раскладывает всё по трём адресам. У монет за валюту всё это происходит в валюте кривой.", "The creator share accrues in the pool and is claimed with a button on the coin page. For ETH coins the pool itself pushes the remaining 30% to the splitter (FeeSplitterV6) on every trade; for quote coins it accrues in the pool and a bot collects it into the splitter once at least $0.10 has built up — the splitter forwards it to the three addresses in the same transaction. For quote coins all of this happens in the curve asset.", "创建者份额在池中累积，在代币页面一键领取。ETH 代币的池子会在每笔交易时自动把其余 30% 推送到分配合约（FeeSplitterV6）；计价货币代币的这部分先在池中累积，累积到 $0.10 后由机器人收集到分配合约，后者在同一笔交易中转给三个地址。计价货币代币的这一切都以曲线资产进行。"),
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      [
        { k: T("Дивиденды и сбор комиссий", "Dividends & fee collection", "分红与手续费收集"), v: T("проверка каждую минуту: выплата держателям от $0.10, сбор доли площадки в сплиттер от $0.10 (пороги — чтобы газ не съедал выплату)", "checks every minute: payouts to holders from $0.10, platform share into the splitter from $0.10 (thresholds keep gas below the payout)", "每分钟检查：向持有者发放 $0.10 起的分红，平台份额累积到 $0.10 后进入分配合约（阈值确保 gas 低于发放额）") },
