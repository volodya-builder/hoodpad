...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      T("При градации пул отправляет резерв и 20% сапплая в мигратор, тот создаёт full-range позицию Uniswap V3 (комиссия 0.3%) и запирает NFT позиции навсегда — ликвидность нельзя вывести никому, включая команду. Перенос запускает бот площадки автоматически, обычно в течение минуты после заполнения кривой; вызвать migrate() у пула может и любой другой.", "At graduation the pool sends its reserve and 20% of supply to the migrator, which creates a full-range Uniswap V3 position (0.3% fee tier) and locks the position NFT forever — nobody, including the team, can withdraw that liquidity. The platform bot triggers the migration automatically, usually within a minute after the curve fills; anyone else can call the pool's migrate() too.", "毕业时，池子将储备金和 20% 总量交给迁移合约，后者创建 Uniswap V3 全区间头寸（0.3% 费率）并永久锁定头寸 NFT——包括团队在内，没有人能提取这部分流动性。迁移由平台机器人自动触发，通常在曲线填满后一分钟内完成；任何人也都可以调用池子的 migrate()。"),
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      [
        ...
        { k: T("Перенос на DEX", "DEX migration", "迁移至 DEX"), v: T("каждые 30 секунд: у монеты с заполненной кривой бот вызывает migrate() пула — ликвидность уезжает в Uniswap V3 и запирается; бот лишь платит газ, деньги идут по коду пула", "every 30 seconds: for a coin whose curve is full the bot calls the pool's migrate() — liquidity moves to Uniswap V3 and is locked; the bot only pays gas, funds move by the pool's code", "每 30 秒：对曲线已满的代币，机器人调用池子的 migrate()——流动性转入 Uniswap V3 并锁定；机器人只支付 gas，资金按池子代码流转") },
