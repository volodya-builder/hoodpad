...
const SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      T("Доля создателя копится в пуле и забирается кнопкой на странице монеты. У ETH-монет остальные 30% пул сам отправляет в сплиттер (FeeSplitterV6) при каждой сделке; у монет за валюту они копятся в пуле, и раз в час бот забирает их в сплиттер, когда накопилось от $1 — сплиттер в той же транзакции раскладывает всё по трём адресам. У монет за валюту всё это происходит в валюте кривой.", "The creator share accrues in the pool and is claimed with a button on the coin page. For ETH coins the pool itself pushes the remaining 30% to the splitter (FeeSplitterV6) on every trade; for quote coins it accrues in the pool and once an hour a bot collects it into the splitter when at least $1 has built up — the splitter forwards it to the three addresses in the same transaction. For quote coins all of this happens in the curve asset.", "创建者份额在池中累积，在代币页面一键领取。ETH 代币的池子会在每笔交易时自动把其余 30% 推送到分配合约（FeeSplitterV6）；计价货币代币的这部分先在池中累积，累积到 $1 后由机器人每小时收集到分配合约，后者在同一笔交易中转给三个地址。计价货币代币的这一切都以曲线资产进行。"),
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      [
        { k: T("Дивиденды и сбор комиссий", "Dividends & fee collection", "分红与手续费收集"), v: T("каждый час: выплата держателям от $1, сбор доли площадки в сплиттер от $1 (порог — чтобы газ не съедал выплату; меньшие суммы копятся и не пропадают)", "hourly: payouts to holders from $1, platform share into the splitter from $1 (the threshold keeps gas below the payout; smaller amounts keep accruing)", "每小时：向持有者发放 $1 起的分红，平台份额累积到 $1 后进入分配合约（阈值确保 gas 低于发放额；较小金额继续累积，不会丢失）") },
