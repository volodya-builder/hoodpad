...
export default function Profile({ wallet, onConnect }) {
  ...
  const [posTab, setPosTab] = useState("pos"); // «Мои позиции» | «История сделок» | «Мои запуски»
        ...
        <>
          ...
          <div className="bottom-card" style={{ marginTop: 18 }}>
            <div className="bt-tabs" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              ...
              <div className={`bt-tab ${posTab === "launch" ? "on" : ""}`} onClick={() => setPosTab("launch")}>
            ...
            {posTab === "launch" && (<>
              {state.launched.length === 0 && (
                <div className="center">{t("Запусков пока нет.")} <a href="#/create">{t("Запустить токен")} →</a></div>
              )}
              {state.launched.length > 0 && (<>
              <div className="prow6 hdr" style={{ gridTemplateColumns: "1.6fr 1fr 1fr 1.4fr 120px", marginTop: 8 }}>
                <span>{t("Токен")}</span>
