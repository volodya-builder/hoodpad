...
export default function CatsGuide({ lang, rarities, onTab, t: tr }) {
  ...
  const faq = [
    ...
    [T("Как редкость влияет на награды?", "How does rarity affect rewards?"),
     T("Редкость задаёт вес кота при каждой раздаче: Обычный ×1, Редкий ×2, Эпический ×3, Мифический ×5, Легендарный ×8. При одной и той же раздаче Легендарный получит в восемь раз больше, чем Обычный. Вес — это доля в конкретной раздаче, а не обещание какой-либо суммы: если комиссий не собралось, раздачи не будет вовсе.",
       "Rarity sets a cat's weight in every distribution: Common x1, Rare x2, Epic x3, Mythic x5, Legendary x8. In the same distribution a Legendary receives eight times what a Common does. Weight is a share of a specific distribution, not a promise of any amount: if no fees were collected, there is no distribution at all.")],
    [T("Зачем раздавать котов бесплатно?", "Why give cats away for free?"),
     T("Чтобы вокруг игры собралось активное комьюнити с самого старта. Бесплатный кот ничего не стоит новому игроку и не создаёт у нас обязательств: он остаётся коллекционным предметом и не даёт доли в платформе, права голоса или гарантированных выплат.",
       "So that an active community forms around the game from day one. A free cat costs a new player nothing and creates no obligation on our side: it stays a collectible and grants no stake in the platform, no voting rights and no guaranteed payouts.")],
