...
export function Terms() {
  ...
  if (lang === "en") {
    return (
      <Doc
        title="Terms of Use"
        updated="Effective date: July 18, 2026 · hood on Robinhood Chain"
        sections={[
          ["1. Agreement",
           "These Terms are a binding agreement between you and the operator of the hood platform (\"hood\", \"we\") governing your use of the hood web interface and related community features. By accessing or using the interface you accept these Terms and the Privacy Policy. If you do not agree, do not use the interface."],
          ["2. About hood",
           "hood is a software interface that lets you interact with autonomous smart contracts on Robinhood Chain: launch tokens with fixed supply, trade on bonding curves, vote and view analytics. hood is not a broker, exchange, custodian or investment adviser. The platform currently operates on Robinhood Chain Testnet: test tokens and test ETH have no monetary value, the contracts have not been audited, and errors are possible."],
          ["3. Eligibility",
           "You may use the interface only if you are at least 18 years old (or the age of majority in your jurisdiction), have full legal capacity, are not subject to sanctions administered by relevant authorities (including OFAC, UN or EU designations), and only where such use is lawful. You are responsible for ensuring that using the interface is legal in your jurisdiction."],
          ["4. Non-Custodial Interface",
           "hood never holds, controls or has access to your assets, private keys or wallets. All operations are executed by smart contracts on the blockchain and initiated by you through your own wallet. We cannot execute, reverse, block or expedite transactions on your behalf."],
          ["5. Wallets and Security",
           "You are solely responsible for the security of your wallet, private keys and seed phrase. hood never asks for them — anyone who does is a scammer. Any action signed by your wallet is deemed your action; we are not liable for losses caused by compromised wallet access."],
          ["6. Transactions",
           "Review every transaction — addresses, amounts, network — before signing. Prices and quotes in the interface are estimates: the final result is determined by the smart contract at execution time and may differ (including due to other users' transactions). Blockchain transactions are irreversible once confirmed."],
          ["7. Token Launches and Content",
           "When you launch a token you are solely responsible for its name, ticker, description, image and links, and you confirm that you hold the necessary rights to them. Token metadata is written to the public blockchain permanently and cannot be edited or deleted by anyone, including hood. We may moderate off-chain content (such as chat) and hide interface listings that violate these Terms, but we cannot alter on-chain records."],
          ["8. Bonding Curve Trading and Graduation",
           "Tokens trade on an automated bonding curve until they reach the graduation threshold, after which liquidity migrates to a decentralized exchange and is locked. Tokens may have little or no liquidity, may be extremely volatile and may become worthless. hood does not guarantee graduation, liquidity, market depth or any price level of any token."],
          ["9. Risk Disclosures",
           "By using the interface you accept the risks of blockchain technology, including: total loss of assets; defects or vulnerabilities in smart contracts (which are unaudited at the test stage); failures of networks, wallets and infrastructure; inaccurate market data; regulatory uncertainty; and tax consequences of your operations. Use the interface only if you understand and accept these risks."],
          ["10. Fees and Taxes",
           "Trades on the curve pay a protocol fee, which the smart contract automatically splits between the token creator, the team and the buyback treasury; the current split is displayed in the interface and in analytics. You also pay network gas fees. You are solely responsible for determining and paying any taxes arising from your operations."],
          ["11. Acceptable Use",
           "You must not use the interface to: violate any applicable law; commit fraud or mislead users; infringe intellectual property; interfere with the operation of the interface or contracts; manipulate markets (including wash trading and coordinated pumping); launder money or finance illegal activity; or launch tokens that constitute securities or other regulated instruments in relevant jurisdictions."],
          ["12. Platform Interests and Conflicts",
           "The team receives a share of protocol fees set in the smart contracts. The buyback treasury is spent on buying back platform tokens at the platform's sole discretion; community voting is advisory and does not create any obligation to buy back any particular token. hood and its team members may hold, buy or sell tokens launched on the platform and may feature certain tokens in the interface. hood owes you no fiduciary duty."],
          ["13. Activity Limits",
          ...
          ["14. Third-Party Services",
           "The interface relies on independent services: wallets, RPC nodes, the block explorer, price APIs, hosting and the chat database. Their availability, accuracy and terms are outside our control; integration does not mean endorsement. Your use of third-party services is governed by their own terms."],
          ["15. Intellectual Property",
           "The interface, its design, texts and code are protected by law. You are granted a limited, non-exclusive, revocable licence to access and use the interface for lawful purposes. Intellectual-property complaints may be sent to the contact in section 26."],
          ["16. Submitted Content",
           "You retain your rights to content you submit (token metadata, chat messages). By submitting it you grant hood a worldwide, royalty-free licence to host, reproduce and display that content for the operation of the platform. Content written to the blockchain is public and permanent by nature."],
          ["17. Feedback",
          ...
          ["18. Service Availability",
           "We may change, suspend or discontinue any part of the interface at any time without notice. We do not guarantee continuous, uninterrupted or error-free operation. Smart contracts remain on the blockchain regardless of the interface's availability."],
          ["19. No Warranties",
           "The interface is provided \"as is\" and \"as available\". To the maximum extent permitted by law we disclaim all warranties, express or implied, including merchantability, fitness for a particular purpose, accuracy of data and uninterrupted or secure operation."],
          ["20. Limitation of Liability",
           "To the maximum extent permitted by law, hood and its contributors are not liable for indirect, incidental, special or consequential damages, or loss of profits, data or assets, arising from use of the interface, contracts, network or third-party services. Our aggregate liability for all claims is limited to the greater of the protocol fees you paid over the preceding 12 months or 100 USD."],
          ["21. Indemnification",
           "You agree to indemnify hood and its contributors against claims, losses and expenses (including reasonable legal fees) arising from your use of the interface, your content, your violation of these Terms or of applicable law. Disputes between users are resolved between users; you release hood from claims related to such disputes."],
          ["22. Termination and Restriction of Access",
           "We may restrict or terminate your access to the interface at any time without notice in case of fraud, market manipulation, sanctions risk, violation of these Terms or a security threat. Restricting interface access does not affect your on-chain assets, which remain under the control of your wallet."],
          ["23. Governing Law and Disputes",
           "These Terms are governed by the law applicable at the operator's location unless mandatory rules of your jurisdiction provide otherwise. Before bringing a formal claim, you agree to attempt good-faith negotiation by contacting us (section 26) and allowing 60 days for resolution. Claims are brought individually; to the extent permitted by law, class actions are waived."],
          ["24. General Terms",
           "If any provision of these Terms is held unenforceable, the remainder stays in effect. You may not assign your rights under these Terms without our consent; we may assign ours in connection with operating or transferring the platform. These Terms and the Privacy Policy are the entire agreement between you and hood regarding the interface."],
  ...
  return (
    <Doc
      ...
      updated="Действуют с: 18 июля 2026 · hood в сети Robinhood Chain"
      sections={[
        ["1. Соглашение",
         "Эти Условия — обязывающее соглашение между вами и оператором платформы hood («hood», «мы»), регулирующее использование веб-интерфейса hood и комьюнити-функций. Получая доступ к интерфейсу, вы принимаете эти Условия и Политику конфиденциальности. Если вы не согласны — не используйте интерфейс."],
        ["2. О hood",
         "hood — программный интерфейс для взаимодействия с автономными смарт-контрактами в сети Robinhood Chain: запуск токенов с фиксированным сапплаем, торговля на бондинг-кривых, голосование и аналитика. hood не является брокером, биржей, кастодианом или инвестиционным советником. Сейчас платформа работает в тестовой сети Robinhood Chain: тестовые токены и тестовый ETH не имеют денежной ценности, контракты не проходили аудит, возможны ошибки."],
        ["3. Право на использование",
         "Использовать интерфейс можно, только если вам исполнилось 18 лет (или вы достигли совершеннолетия в своей юрисдикции), вы полностью дееспособны, не находитесь под санкциями соответствующих органов (включая списки OFAC, ООН и ЕС) и только там, где такое использование законно. Вы сами отвечаете за то, что использование интерфейса разрешено в вашей юрисдикции."],
        ["4. Некастодиальный интерфейс",
         "hood никогда не хранит и не контролирует ваши активы, приватные ключи и кошельки и не имеет к ним доступа. Все операции исполняются смарт-контрактами в блокчейне и инициируются вами через ваш собственный кошелёк. Мы не можем провести, отменить, заблокировать или ускорить транзакцию за вас."],
        ["5. Кошельки и безопасность",
         "Вы единолично отвечаете за сохранность кошелька, приватных ключей и сид-фразы. hood никогда их не запрашивает — любой, кто просит, мошенник. Любое действие, подписанное вашим кошельком, считается вашим действием; мы не отвечаем за убытки из-за компрометации доступа к кошельку."],
        ["6. Транзакции",
         "Проверяйте каждую транзакцию — адреса, суммы, сеть — до подписания. Цены и котировки в интерфейсе являются оценками: итоговый результат определяет смарт-контракт в момент исполнения, и он может отличаться (в том числе из-за транзакций других пользователей). Подтверждённые транзакции в блокчейне необратимы."],
        ["7. Запуск токенов и контент",
         "Запуская токен, вы единолично отвечаете за его название, тикер, описание, картинку и ссылки и подтверждаете, что обладаете необходимыми правами на них. Метаданные токена записываются в публичный блокчейн навсегда — их не может изменить или удалить никто, включая hood. Мы можем модерировать офчейн-контент (например, чат) и скрывать из интерфейса листинги, нарушающие эти Условия, но не можем менять ончейн-записи."],
        ["8. Торговля на кривой и градация",
         "Токены торгуются на автоматической бондинг-кривой до порога градации, после чего ликвидность мигрирует на децентрализованную биржу и запирается. Токены могут иметь низкую ликвидность или не иметь её, быть крайне волатильными и полностью обесцениться. hood не гарантирует градацию, ликвидность, глубину рынка или какой-либо уровень цены любого токена."],
        ["9. Раскрытие рисков",
         "Используя интерфейс, вы принимаете риски блокчейн-технологий, включая: полную потерю активов; дефекты и уязвимости смарт-контрактов (на тестовом этапе они не аудированы); сбои сетей, кошельков и инфраструктуры; неточность рыночных данных; регуляторную неопределённость; налоговые последствия ваших операций. Используйте интерфейс, только если понимаете и принимаете эти риски."],
        ["10. Комиссии и налоги",
         "Сделки на кривой платят комиссию протокола, которую смарт-контракт автоматически делит между создателем токена, командой и казной выкупа; актуальное распределение отображается в интерфейсе и аналитике. Вы также оплачиваете сетевой газ. Вы единолично отвечаете за определение и уплату налогов, возникающих из ваших операций."],
        ["11. Допустимое использование",
         "Запрещено использовать интерфейс для: нарушения применимого закона; мошенничества и введения пользователей в заблуждение; нарушения интеллектуальных прав; вмешательства в работу интерфейса и контрактов; манипулирования рынком (включая wash-трейдинг и скоординированные пампы); отмывания средств и финансирования незаконной деятельности; запуска токенов, являющихся ценными бумагами или иными регулируемыми инструментами в соответствующих юрисдикциях."],
        ["12. Интересы платформы и конфликты",
         "Команда получает долю комиссий протокола, заданную в смарт-контрактах. Казна выкупа тратится на выкуп токенов платформы по единоличному усмотрению платформы; голосование комьюнити носит совещательный характер и не создаёт обязанности выкупать какой-либо токен. hood и члены команды могут держать, покупать и продавать токены, запущенные на платформе, и выделять отдельные токены в интерфейсе. hood не несёт перед вами фидуциарных обязанностей."],
        ["13. Лимиты активности",
        ...
        ["14. Сторонние сервисы",
         "Интерфейс опирается на независимые сервисы: кошельки, RPC-узлы, эксплорер, API цен, хостинг и базу чата. Их доступность, точность и условия вне нашего контроля; интеграция не означает одобрения. Использование сторонних сервисов регулируется их собственными условиями."],
        ["15. Интеллектуальная собственность",
         "Интерфейс, его дизайн, тексты и код защищены законом. Вам предоставляется ограниченная, неисключительная, отзывная лицензия на доступ к интерфейсу и его использование в законных целях. Жалобы на нарушение интеллектуальных прав направляйте на контакт из раздела 26."],
        ["16. Пользовательский контент",
         "Права на контент, который вы публикуете (метаданные токенов, сообщения чата), остаются за вами. Публикуя его, вы предоставляете hood всемирную безвозмездную лицензию на хранение, воспроизведение и показ этого контента для работы платформы. Контент, записанный в блокчейн, публичен и постоянен по своей природе."],
        ["17. Обратная связь",
        ...
        ["18. Доступность сервиса",
         "Мы можем изменять, приостанавливать или прекращать работу любой части интерфейса в любой момент без уведомления. Мы не гарантируем непрерывную и безошибочную работу. Смарт-контракты остаются в блокчейне независимо от доступности интерфейса."],
        ["19. Отказ от гарантий",
         "Интерфейс предоставляется «как есть» и «как доступно». В максимальной степени, допустимой законом, мы отказываемся от любых гарантий, явных и подразумеваемых, включая товарную пригодность, соответствие определённой цели, точность данных и бесперебойную или безопасную работу."],
        ["20. Ограничение ответственности",
         "В максимальной степени, допустимой законом, hood и участники разработки не отвечают за косвенные, случайные, специальные и последующие убытки, упущенную выгоду, потерю данных или активов, возникшие из использования интерфейса, контрактов, сети или сторонних сервисов. Совокупная ответственность по всем требованиям ограничена большей из сумм: комиссии протокола, уплаченные вами за предыдущие 12 месяцев, или 100 долларов США."],
        ["21. Возмещение убытков",
         "Вы обязуетесь оградить hood и участников разработки от претензий, убытков и расходов (включая разумные юридические издержки), возникших из вашего использования интерфейса, вашего контента, нарушения вами этих Условий или закона. Споры между пользователями решаются между пользователями; вы освобождаете hood от претензий, связанных с такими спорами."],
        ["22. Прекращение и ограничение доступа",
         "Мы можем ограничить или прекратить ваш доступ к интерфейсу в любой момент без уведомления в случае мошенничества, манипулирования рынком, санкционного риска, нарушения этих Условий или угрозы безопасности. Ограничение доступа к интерфейсу не затрагивает ваши ончейн-активы — они остаются под контролем вашего кошелька."],
        ["23. Применимое право и споры",
         "Эти Условия регулируются правом, применимым по месту нахождения оператора, если императивные нормы вашей юрисдикции не предусматривают иное. До предъявления формального требования вы соглашаетесь попытаться урегулировать спор добросовестными переговорами, связавшись с нами (раздел 26) и дав 60 дней на решение. Требования предъявляются в индивидуальном порядке; в допустимой законом мере коллективные иски исключаются."],
        ["24. Общие положения",
         "Если какое-либо положение этих Условий признано неисполнимым, остальные сохраняют силу. Вы не можете уступать свои права по этим Условиям без нашего согласия; мы можем уступить свои в связи с управлением платформой или её передачей. Эти Условия вместе с Политикой конфиденциальности составляют полное соглашение между вами и hood в отношении интерфейса."],
