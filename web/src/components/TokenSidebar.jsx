...
export default function TokenSidebar({ current }) {
  ...
  if (sort.k === "raised") list.sort((a, b) => Number(a.reserve - b.reserve) * dir);
  if (sort.k === "new" && sort.d === 1) list.reverse(); // базово новые первыми
  ...
  return (
    <div className="tok-sidebar">
      ...
      <div className="ts-hdr">
        ...
        <span className="sort-h" onClick={() => pick("vol")}>{t("Объём")}<i>{arrow("vol")}</i></span>
        <span className="sort-h ts-right" onClick={() => pick("mcap")}>{t("Капа")}<i>{arrow("mcap")}</i></span>
        <span className="sort-h ts-right" onClick={() => pick("chg")}>{t("24ч")} %<i>{arrow("chg")}</i></span>
      </div>
