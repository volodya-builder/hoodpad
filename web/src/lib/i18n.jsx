...
const EN = {
  ...
  "Один кошелёк — максимум 25% объёма токена": "One wallet — at most 25% of a token's volume",
  ...
  "Один кошелёк даёт максимум 25% честного объёма токена — сибил из одного кита не работает.": "One wallet contributes at most 25% of a token's fair volume — a single-whale sybil doesn't work.",
  ...
  "Очки боя = ЧЕСТНЫЙ объём за день × (1 + прирост капитализации). Честный объём: покупки минус продажи по каждому кошельку, сделки создателя не в счёт, один кошелёк — максимум 25%. Накрутка и прокрутка объёма очков не дают, дамп цены режет их. На каждом чекпоинте вылетает токен с наименьшими очками.": "Battle score = FAIR daily volume × (1 + market cap growth). Fair volume: buys minus sells per wallet, creator trades excluded, one wallet capped at 25%. Wash trading earns no points; dumping cuts them. At each checkpoint the lowest-scoring token is eliminated.",
