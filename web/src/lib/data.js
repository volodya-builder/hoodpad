...
const SUBGRAPH_VERSIONS = ["5.0.0", "4.0.2"]; // 5.0.0: комплект 17.09.2026 (новые фабрики + казны V2); 4.0.2 — запасной, пока 5.0.0 синхронизируется; // 4.0.2: трейдер через зап = отправитель tx (PnL); первая отвечающая версия // 4.0.0 был с graft от старых фабрик — не использовать
