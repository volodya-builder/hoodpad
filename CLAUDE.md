...
- **Политика трат казны (решение владельца 06.08.2026):** комиссия 1% делится контрактами 50% создателю / 20% команде / **30% в казну**; дальше казна распределяется по направлениям: **20% баланса — коты** (покупка токенизированных акций и раздача держателям NFT), **10% — арена** (выкуп токенов-призёров подиума и сжигание), **70% — выкупы по итогам голосования** со сжиганием. Это параметры политики, а не контракта: исполняет ИИ-казначей, менять можно без передеплоя. Единый источник правды во фронте — `TREASURY_POLICY` в web/src/lib/config.js. Прежняя политика (подиум 50% ежедневно, голосование 20%, Гранд-Арена 30%) отменена. Очки арены и допуск к голосованию — по «честному объёму» (web/src/lib/fairvol.js: |покупки−продажи| на кошелёк, создатель не в счёт, кэп 25%; голосу нужно $500 и 2+ дня торговли). Ядро арены общее для сайта и бота: web/src/lib/arena-core.js.
...
## Коты-брокеры и RWA-запуски (готово к деплою, 05.08.2026)
