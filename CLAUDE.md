...
продать через зап, активность, чат), «Создать» (ETH или акция из белого
списка, ставка выплат 0–3%), **«Арена»** (суточный турнир, вернули 15.09.2026:
`pages/Arena.jsx`, ядро `lib/arena-core.js`, приз — казна арены), «Аналитика»
(+ лидеры), профиль, трейдер, политика/условия.

**Спрятано флагами `FEATURES` в `web/src/lib/config.js`, код не удалён — не
работать над этим и не «чинить»:** **ai** (вкладка «ИИ», доска идей, чат с ИИ,
выбор модели при создании, воркер — выключено владельцем 15.09.2026), about,
grandArena, cats, vote, treasury, ticker, headerSearch, netSwitch,
weeklyWorkshop, taxToken, customQuote, payInQuote, trustScore,
creatorSellBanner, activityTab (вкладка «Активность» в нижней таблице монеты).
Скрытые страницы грузятся лениво (`lazy()` в App.jsx), их хуки выключены
флагами — в сеть не ходят.
...
- **Ждут деплоя владельцем** (`node scripts/deploy-arena-economy.js --deploy`,
  потом через 48ч `--apply`): FeeSplitterV5 (`contracts/FeeSplitterV5.sol`,
  арене 2/4, команде 1/4, остаток агенту/создателю) и ArenaTreasury
  (`contracts/ArenaTreasury.sol`, деньги уходят только на выкуп+сжигание;
...
## Экономика (решение владельца 15.09.2026)

Комиссия сделки 1%. Новые монеты: **70% создателю / 20% в казну арены / 10%
команде** (с ИИ, если его вернут: 60 / 20 / 10 / 10 агенту). Пул отдаёт
создателю 60% (зашито в пару при создании, навсегда), остальные 40% —
FeeSplitterV5: половина арене, четверть команде, четверть агенту если `aiOf`,
иначе создателю. Монеты с 7000 (запущены 14–15.09) шлют 30%: арене 15 /
команде 7.5 / создателю 7.5; старые с 5000 — 50%: 25 / 12.5 / 12.5.
Пока V5 не задеплоен, на цепи действует схема 14.09: 80/20 (V4).
Арена: раз в сутки бот `bot/arena/arena.mjs` (workflow arena.yml, эстафета)
делит весь ETH казны арены между вчерашним подиумом 70/20/10 — выкуп у кривой
(ETH-монеты) или через зап (монеты за валюту) и сжигание; состояние — события
`Buyback` казны с пометкой «arena <день> <место>». Подиум — `lib/arena-core.js`,
общий с сайтом. **Пока в арене только ETH-монеты** (сабграф индексирует одну
ETH-фабрику); монеты за валюту войдут, когда сабграф начнёт индексировать
фабрику за валюту. Выплаты холдерам: у монет за валюту ставка 0–3% с каждой
...
- ИИ монет выключен (FEATURES.ai): воркер чата не задеплоен, агент без расписания.
- Арена без монет за валюту, пока сабграф не индексирует quote-фабрику;
  V5/ArenaTreasury ждут деплоя владельцем, до этого фонд на странице — «скоро».
