...
- ⚠ **Не исправлено (до первой градации!):** `UniswapV3Migrator.migrate()` инициализирует пул Uniswap только если он ещё не создан, а `amount0Min/amount1Min = 0`. Атакующий может заранее создать пул с искажённой ценой и при градации забрать значительную часть 6.5 ETH + 200M токенов. Чинить: проверять `slot0().sqrtPriceX96` после создания + реальные min-суммы; требует передеплоя мигратора (аудит 27.07.2026).
