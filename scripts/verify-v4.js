 ...
 *
 * Что делает:
 *   1. берёт адреса из scripts/relaunch-v4-output.json (после перезапуска)
 ...
 *
 * Запуск:  node scripts/verify-v4.js            # всё из relaunch-v4-output.json
 *          node scripts/verify-v4.js --only zap # только один
 *          EXTRA="Name=0x…,Name2=0x…" node scripts/verify-v4.js   # добавить адреса
...
async function targets() {
  ...
  if (out) {
    const map = { migrator: "UniswapV3Migrator", factory: "LaunchpadFactoryV3", migratorQ: "UniswapV3MigratorQuote", quoteFactory: "LaunchpadFactoryQuoteV3", zap: "CurveZap", arena: "ArenaTreasuryV3", hoodTreasury: "ArenaTreasuryV3", splitter: "FeeSplitterV6" };
    ...
    try {
      ...
      for (const [fk, tokName, poolName] of [["factory", "LaunchToken", "BondingCurvePoolV3"], ["quoteFactory", "DividendToken", "BondingCurvePoolQuoteV3"]]) {
...
async function main() {
  ...
  if (bad.length) { console.log(`\nНе верифицированы: ${bad.length}. Повтори позже: node scripts/verify-v4.js --only <имя>`); process.exit(1); }
